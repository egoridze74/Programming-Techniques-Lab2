...
plt.plot(df['size'], df['bst_ms'], label='BST (O(log n) avg)', marker='s', linewidth=2, markersize=4)
plt.plot(df['size'], df['rbt_ms'], label='Red-Black Tree (O(log n))', marker='^', linewidth=2, markersize=4)
plt.plot(df['size'], df['hash_ms'], label='Hash Table (O(1) avg)', marker='d', linewidth=2, markersize=4)
plt.plot(df['size'], df['multimap_ms'], label='std::multimap (RBT)', marker='*', linewidth=2, markersize=4)

plt.xscale('log')
plt.yscale('log')
plt.xlabel('Размер массива N (логарифм. шкала)', fontsize=12)
plt.ylabel('Время поиска (ms, логарифм. шкала)', fontsize=12)
...
print("-"*60)
for method in ['linear_ms', 'bst_ms', 'rbt_ms', 'hash_ms', 'multimap_ms']:
...
# Дополнительный график: сравнение структур данных (без линейного поиска)
plt.figure(figsize=(12, 6))
plt.plot(df['size'], df['bst_ms'], label='BST', marker='s', linewidth=2)
plt.plot(df['size'], df['rbt_ms'], label='Red-Black Tree', marker='^', linewidth=2)
plt.plot(df['size'], df['hash_ms'], label='Hash Table', marker='d', linewidth=2)
plt.plot(df['size'], df['multimap_ms'], label='std::multimap', marker='*', linewidth=2)

plt.xscale('log')
plt.yscale('log')
plt.xlabel('Размер массива N', fontsize=12)
plt.ylabel('Время поиска (ms)', fontsize=12)
plt.title('Сравнение эффективных структур данных (без линейного поиска)', fontsize=14)
plt.legend(fontsize=10)
plt.grid(True, alpha=0.3)
plt.savefig('search_effective_only.png', dpi=150)
plt.show()